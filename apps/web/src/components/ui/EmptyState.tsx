type IconName = 'calendar' | 'inbox' | 'chat' | 'receipt' | 'bell' | 'search' | 'star' | 'users';
...
export default function EmptyState({ icon, title, description, ctaLabel, onCta }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center text-center py-16 gap-3 px-4">
      <div className="text-stone-500 mb-1">{icons[icon]}</div>
      <p className="text-base font-medium text-stone-400">{title}</p>
      {description && <p className="text-sm text-stone-500 max-w-xs">{description}</p>}
      {ctaLabel && onCta && (
        <button
          onClick={onCta}
          className="mt-2 bg-rose-600 hover:bg-rose-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
        >
          {ctaLabel}
        </button>
      )}
